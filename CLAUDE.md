...
## What This Is
OpenEntropy harvests hardware entropy from 30 unconventional sources on consumer devices. Rust workspace with Python bindings.
...
- **Fast sources by default.** CLI commands use 18 fast sources (<2s). `--sources all` for everything.
- **Min-entropy (H∞) over Shannon.** Grading based on NIST SP 800-90B min-entropy, not Shannon which overestimates.
- **159 tests, 0 clippy warnings.** Keep it that way.
...
22 tests are `#[ignore]` because they require specific hardware (camera, BLE, WiFi, etc.).

## Platform
Primary: macOS Apple Silicon (M1-M4). 26/30 sources available on Mac Mini, 30/30 on MacBook.

...
## What This Is
OpenEntropy harvests hardware entropy from 35 unconventional sources on consumer devices. Rust workspace with Python bindings.
...
- **Fast sources by default.** CLI commands use 23 fast sources (<2s). `--sources all` for everything.
- **Min-entropy (H∞) over Shannon.** Grading based on NIST SP 800-90B min-entropy, not Shannon which overestimates.
- **188 tests, 0 clippy warnings.** Keep it that way.
...
- **Frontier** (5): amx_timing, thread_lifecycle, mach_ipc, tlb_shootdown, pipe_buffer
...
27 tests are `#[ignore]` because they require specific hardware (camera, BLE, WiFi, etc.).

...
pub fn run(source_name: &str) {
    ...
    let matches: Vec<_> = sources
        .into_iter()
        .filter(|s| s.name().to_lowercase().contains(&source_name.to_lowercase()))
        .collect();

    if matches.is_empty() {
        eprintln!("Source '{}' not found. Run 'scan' to list sources.", source_name);
        std::process::exit(1);
    }
    ...
    println!("  Shannon entropy: {:.4} / 8.0 bits", quality.shannon_entropy);

...
//!
//! Run: cargo run --example full_source_audit --release -p openentropy-core

use openentropy_core::{EntropySource, detect_available_sources};
...
fn pearson(a: &[u8], b: &[u8]) -> f64 {
    ...
    let sa = a[..n]
        .iter()
        .map(|&x| (x as f64 - ma).powi(2))
        .sum::<f64>()
        .sqrt();
    let sb = b[..n]
        .iter()
        .map(|&x| (x as f64 - mb).powi(2))
        .sum::<f64>()
        .sqrt();
    if sa * sb == 0.0 {
        return 0.0;
...
fn main() {
    ...
    const SKIP_COLLECT: &[&str] = &[
        ...
        "rndr_trap_timing",    // hangs (may block waiting for hardware entropy)
    ];
    ...
    for src in &sources {
        ...
        if SKIP_COLLECT.iter().any(|&s| s == info.name) {
            ...
            println!(
                "{:<32} {:<14} {:<8} {:<8} {} ← SKIP (known crash/hang)",
                info.name,
                cat,
                "?",
                "?",
        ...
        let n = match info.name {
            "timer_coalescing" | "usb_enumeration" | "usb_timing" | "nvme_latency"
            | "fsync_journal" | "keychain_timing" | "kqueue_events" => 200,
    ...
    let keywords = [
        "aes", "timing", "smc", "usb", "pll", "mach", "clock", "cache", "crypto",
    ];
    for kw in &keywords {

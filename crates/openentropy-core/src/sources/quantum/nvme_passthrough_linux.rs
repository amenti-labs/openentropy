...
//!
//! `quantum_fraction ≈ 0.45` — closest to hardware achievable from userspace on
//! Linux. The filesystem and block layers are completely eliminated. Remaining
//! classical noise is from NVMe driver submission/completion overhead (~4us) and
//! controller firmware. The NAND physics component (charge sensing, tunneling)
//! is a larger fraction of the total timing than any other software-accessible
//! approach.
...
static NVME_PASSTHROUGH_INFO: SourceInfo = SourceInfo {
    ...
    physics: "Submits NVMe admin commands (Get Log Page for SMART/Health Information, Log ID 02h) \
              ...
              collection scheduling) and NAND flash page access. NAND reads involve charge sensing \
              where threshold voltage depends on trapped electron count from Fowler-Nordheim \
              tunneling \u{2014} a quantum mechanical process. SMART temperature values provide \

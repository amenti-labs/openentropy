...
//!
//! `quantum_fraction ≈ 0.40` — filesystem layer removed, but NVMe controller
//! firmware (FTL, GC, wear leveling) still contributes significant classical
//! timing variance. The NAND charge sensing component has genuine quantum
//! origin (electron tunneling probability).
...
static NVME_RAW_DEVICE_INFO: SourceInfo = SourceInfo {
    ...
    physics: "Reads directly from raw block devices (/dev/rdiskN on macOS, /dev/nvmeXnYpZ on \
              ...
              involve charge sensing: the threshold voltage of each cell depends on the number \
              of electrons trapped in the floating gate via Fowler-Nordheim tunneling. The \
              sensing time varies with trapped charge, which is a quantum mechanical quantity.",

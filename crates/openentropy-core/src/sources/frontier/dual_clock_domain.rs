...
static DUAL_CLOCK_DOMAIN_INFO: SourceInfo = SourceInfo {
    ...
    entropy_rate_estimate: 12000.0, // CV=704% with fast sampling
    composite: false,
};
    ...
    impl EntropySource for DualClockDomainSource {
        ...
        fn collect(&self, n_samples: usize) -> Vec<u8> {
            unsafe {
                ...
                let mut beats = Vec::with_capacity(raw_count);

                for _ in 0..raw_count {
                    // Read the 24 MHz CNTVCT
                    let cntvct = mach_time();
                    ...
                    // bits[23:0] of (CNTVCT XOR SoC_A) encodes the phase
                    // between the 24 MHz and 41 MHz oscillators.
                    // Further XOR with SoC_B adds the intra-41MHz-domain beat.
                    let beat = (cntvct ^ soc_a ^ soc_b) & 0xFFFFFF;
                    beats.push(beat);
                }

                // Beat values have CV=704.2% — no shift needed, entropy in all bits
...
mod tests {
    ...
    fn info() {
        ...
        // Should have the highest entropy rate estimate in the library (CV=704%)
        assert!(src.info().entropy_rate_estimate > 10000.0);
    }

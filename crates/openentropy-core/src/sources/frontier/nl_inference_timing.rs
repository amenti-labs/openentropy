...
mod objc_nl {
    ...
    unsafe extern "C" {
        ...
        // NB: objc_msgSend is variadic; Rust can call it with extra args via cast.
...
mod imp {
    ...
    impl EntropySource for NLInferenceTimingSource {
        ...
        fn collect(&self, n_samples: usize) -> Vec<u8> {
            // 6× oversampling: each measurement has high individual entropy but
            // consecutive samples can be correlated when cache stays warm.
            let raw_count = n_samples * 6 + 64;
            ...
            let alloc = unsafe { objc_msgSend(class, alloc_sel) };
            let rec = unsafe { objc_msgSend(alloc, init_sel) };
            if rec.is_null() {
                return Vec::new();
            }

            // Warm up: let the recognizer load its model and populate the cache.
            for i in 0..16_usize {
                ...
                unsafe {
                    objc_msgSend(rec, process_sel, ns_str);
                    objc_msgSend(rec, dominant_sel);
                    objc_msgSend(rec, reset_sel);
                };
            }
            ...
            for i in 0..raw_count {
                ...
                unsafe {
                    objc_msgSend(rec, process_sel, ns_str);
                    let lang = objc_msgSend(rec, dominant_sel);
                    objc_msgSend(rec, reset_sel);
